...
class TicketSelectAdapter(private val context: Context) : ListAdapter<Ticket, TicketSelectAdapter.TicketHolder>(TicketDiffCallback()) {
	...
	override fun onBindViewHolder(holder: TicketHolder, position: Int) {
		...
		if (context is TicketSelectorActivity) {
			holder.root.setOnClickListener {
				(context).setResult(currentTicket.uid)
			}
		}
	}
